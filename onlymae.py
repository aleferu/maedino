...
class MAEViTSeg(nn.Module):
    def __init__(
        ...
        # Initialize ViT Backbone via timm
    ...
    def forward(self, x):
        ...
        # Decode to Masks
        masks_2d = self.decoder(feat_mae)  # Shape: (B*D, Num_Classes, H, W)
...
def main():
    parser = argparse.ArgumentParser(description="Train MAEViTSeg with 5-fold CV")
    parser.add_argument(
        "--experiment",
        required=True,
        choices=["Exp_MAE_Frozen", "Exp_MAE_Unfrozen"],
    ...
    experiments = [
        {"name": "Exp_MAE_Frozen",   "freeze_mae": True,  "batch_size": 5},
        {"name": "Exp_MAE_Unfrozen", "freeze_mae": False, "batch_size": 1},
    ]
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data_dicts)):
        ...
        model = MAEViTSeg(freeze_mae=freeze_mae).to(device)
