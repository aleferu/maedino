...
class DINOViTSeg(nn.Module):
    ...
    def forward(self, x):
        ...
        # Decode to Masks
        masks_2d = self.decoder(feat_dino)  # Shape: (B*D, Num_Classes, H, W)
...
def main():
    parser = argparse.ArgumentParser(description="Train DINOViTSeg with 5-fold CV")
    parser.add_argument(
        "--experiment",
        required=True,
        choices=["Exp_Dino_Frozen", "Exp_Dino_Unfrozen"],
    ...
    experiments = [
        {"name": "Exp_Dino_Frozen",   "freeze_dino": True,  "batch_size": 5},
        {"name": "Exp_Dino_Unfrozen", "freeze_dino": False, "batch_size": 1},
    ]
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(data_dicts)):
        ...
        model = DINOViTSeg(freeze_dino=freeze_dino).to(device)
